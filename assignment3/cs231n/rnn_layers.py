...
def rnn_backward(dh, cache):
    ...
    dx[:, T-1, :], dprev_h[:, T-1, :], dWx, dWh, db = rnn_step_backward(dh[:, T-1, :], cache[T-1])

    for t in reversed(range(T - 1)):
        dx[:, t, :], dprev_h[:, t, :], dWxt, dWht, dbt = rnn_step_backward(dh[:, t, :] + dprev_h[:, t + 1, :], cache[t])
        dWx += dWxt
        dWh += dWht
        db += dbt
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    cache = x, prev_h, Wx, Wh, a, i, f, o, g, prev_c, next_c
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    x, prev_h, Wx, Wh, a, i, f, o, g, prev_c, next_c = cache
    H = Wh.shape[0]
    ...
    dsigmoid = sigmoid(a[:, 0:3*H]) * (1 - sigmoid(a[:, 0:3*H]))
    da = np.zeros_like(a)
    da[:, 0:H] = dnext_c * g * dsigmoid[:, 0:H]
    da[:, H:2*H] = dnext_c * prev_c * dsigmoid[:, H:2*H]
    da[:, 2*H:3*H] = dnext_h * np.tanh(next_c) * dsigmoid[:, 2*H:3*H]
    da[:, 3*H:4*H] = dnext_c * i * (1 - np.tanh(a[:, 3*H:4*H])**2)
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(T):
        ...
        h[:, t+1, :], c[:, t+1, :], cache[t] = lstm_step_forward(xt, ht, ct, Wx, Wh, b)
...
def lstm_backward(dh, cache):
    ...
    dprev_c = np.zeros((N, T+1, H))

    for t in reversed(range(T)):
        dht = dh[:, t, :] + dprev_h[:, t+1, :]
        dnext_ct = dprev_c[:, t+1 , :]

        dx[:, t, :], dprev_h[:, t, :], dprev_c[:, t, :], dWxt, dWht, dbt = lstm_step_backward(dht, dnext_ct, cache[t])
        dWx += dWxt
        dWh += dWht
        db += dbt
