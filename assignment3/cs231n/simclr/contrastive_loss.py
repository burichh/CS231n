...
def sim(z_i, z_j):
    ...
    norm_dot_product = z_i.dot(z_j.T) / (torch.linalg.norm(z_i) * torch.linalg.norm(z_j))
...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        denominator = 0
        other_indices = [j for j in range(2*N) if j != k]
        for j in other_indices:
            denominator += torch.exp(sim(z_k, out[j]) / tau)
        l_k_kN = -torch.log(torch.exp(sim(z_k, z_k_N) / tau) / denominator)


        denominator = 0
        other_indices = [j for j in range(2*N) if j != k + N]
        for j in other_indices:
            denominator += torch.exp(sim(z_k_N, out[j]) / tau)
        l_kN_k = -torch.log(torch.exp(sim(z_k_N, z_k) / tau) / denominator)
...
def sim_positive_pairs(out_left, out_right):
    ...
    pos_pairs = torch.sum(out_left * out_right, axis=1, keepdim=True)
...
def compute_sim_matrix(out):
    ...
    sim_matrix /= (torch.linalg.norm(out, dim=1, keepdim=True).matmul(torch.linalg.norm(out, dim=1, keepdim=True).T))
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    numerator = numerator.repeat(2, 1)
