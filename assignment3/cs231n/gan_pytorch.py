...
def discriminator_loss(logits_real, logits_fake):
    ...
    true_labels = torch.ones(logits_real.size()).type(dtype).squeeze()
...
def generator_loss(logits_fake):
    ...
    true_labels = torch.ones(logits_fake.size()).type(dtype).squeeze()
...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    loss = 0.5 * torch.mean((scores_real - 1).pow(2) + scores_fake.pow(2))
