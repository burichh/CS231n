...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        query = self.query(query).reshape((N, S, H, D)).permute((0, 2, 1, 3))
        key = self.key(key).reshape((N, T, H, D)).permute((0, 2, 3, 1))
        value = self.value(value).reshape((N, T, H, D)).permute((0, 2, 1, 3))

        attention = query.matmul(key) / math.sqrt(D) # (N, H, S, T)
        if attn_mask != None:
          attention.masked_fill_(attn_mask == False, -float("inf"))
        attention = self.attn_drop(F.softmax(attention, dim=3))

        output = attention.matmul(value) # (N, H, S, D)
        output = output.transpose(1, 2) # (N, S, H, D)
